...
use {
    crate::{
        ...
        r_time::AsRawTime,
        ruby_sys::{
            rb_fd_init, rb_fd_set, rb_fd_term, rb_fdset_t, rb_io_ascii8bit_binmode, rb_io_close,
            rb_io_eof, rb_io_fdopen, rb_io_flush, rb_io_getbyte, rb_io_gets, rb_io_write, rb_pipe,
...
impl Rio {
    ...
    /// _Inefficiently_ write contents to one-off RString buffer, then into IO.
    /// Return bytes written, this means partial writes is possible.
    ///
    /// TODO: This should be done via ::IO::Buffer when it's stable
    pub fn write_str<S>(&self, buf: S) -> Result<usize, Error>
    where
        S: Into<RString>,
    {
        protect(|| {
            Value::new(unsafe { rb_io_write(self.as_rb_value(), buf.into().as_rb_value()) })
        })?
        .try_convert()
    }
}
...
///
/// Allows `::select` to operate over more than 1024 FDs.
pub struct FDset {
    inner: rb_fdset_t,
}
...
impl FDset {
    ...
    ///
    /// Except `FDset` operates on more than the historical limit of 1024 FDs.
    ...
    pub fn select(
        ...
        let count = reads.len() + writes.len() + errors.len();
